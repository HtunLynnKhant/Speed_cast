...
class ClientLoginController extends Controller
{
    ...
    public function showLogin(): View|RedirectResponse
    {
        if (Auth::check() && Auth::user()->role === 'client') {
            ...
            return redirect()->route('client.dashboard')->with('success', 'You are already logged in!');
        }

        // Show the login view if the user is not authenticated
        return view('client.Auth.login'); // Ensure this view path exists
    }
        ...
        if (Auth::attempt($credentials)) {
            ...
            if ($user->role === 'client') {
                ...
                if ($user->password_changed === 0) {
                    session(['showpasswordModal' => true]);  // Set session variable to show modal

                    return redirect()->route('client.dashboard');
                }

                // If password was already changed, proceed normally
                return redirect()->route('client.dashboard')->with('success', 'You are logged in as Client!');
            } else {
                // If the user is not a client, log them out
                Auth::logout();

                return redirect()->route('clientlogin')->with('error', 'You do not have client access.');
            }
        }

        // If authentication fails, redirect back with an error message
        return redirect()->route('clientlogin')->with('error', 'Invalid email or password!');
    }
    ...
    public function updatePassword(Request $request)
    {
        ...
        session()->forget('showpasswordModal'); // Clear session variable after password change
